...
function renderGrid(templates) {
  ...
  for (var i = 0; i < templates.length; i++) {
    var t = templates[i];
    var fallback = 'background:linear-gradient(135deg,' + t.primaryColor + '14,' + t.inkColor + '06);color:' + t.inkColor + ';font-family:' + esc(t.fontFamily || 'var(--font-display)') + ', system-ui, -apple-system, sans-serif';
    html += '<div class="template-card" data-slug="' + t.slug + '">';
    if (t.hasPreview) {
      html += '<div class="card-preview" data-preview-slug="' + t.slug + '"></div>';
    } else {
      html += '<div class="card-preview"><div class="card-preview-placeholder" style="' + fallback + '">' + monogram(t.name) + '</div></div>';
    }
