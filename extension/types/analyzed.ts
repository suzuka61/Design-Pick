...
export interface PageBackground {
  hex: string;
  /** Text color that pairs with this background (from body computed style). */
  textHex: string;
  rgb: { r: number; g: number; b: number };
  /** Relative luminance (0-255). < 128 = dark. */
  luminance: number;
  isDark: boolean;
}
...
export interface AnalyzedColorPalette {
  ...
  /** Actual rendered page background — drives Quick Reference & theme decisions. */
...
// --- New types from Skill doc fusion ---
...
export interface Constraint {
  ...
  evidence: string;
  confidence: number;
  source: 'typo' | 'color' | 'spacing' | 'shape' | 'component';
}
