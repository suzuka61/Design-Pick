...
import type { TokenNameMap, ColorTokenEntry } from '../analyzer/token-namer';
...
// --- i18n: UI strings (Chinese only) ---
const I18N: Record<string, string> = {
  navMission:          '使命',
  navCharacteristics:  '风格特征',
  navColors:           '颜色',
  navTypography:       '字体',
  navButtons:          '按钮',
  navSpacing:          '间距',
  ...
  navResponsive:       '响应式',
  heroAccent:          '源自',
  heroSub:             '所有颜色、字体、组件——自动提取并可视化。',
  heroSource:          '来源',
  labelMission:        '01 / 使命与品牌',
  labelChar:           '02 / 风格与特征',
  labelColors:         '03 / 颜色',
  labelTypography:     '04 / 字体',
  labelButtons:        '05 / 按钮',
  labelSpacing:        '06 / 间距',
  labelRadius:         '圆角',
  labelElevation:      '07 / 深度',
  labelA11y:           '08 / 无障碍',
  labelMotion:         '09 / 动效与过渡',
  labelDos:            '10 / 准则',
  labelAnti:           '11 / 反模式',
  labelQA:             '12 / QA 清单',
  labelResponsive:     '13 / 响应式',
  titleMission:        '使命与品牌',
  titleChar:           '风格与特征',
  titleColors:         '色板',
  titleTypography:     '字体尺度',
  titleButtons:        '按钮变体',
  titleSpacing:        '间距尺度',
  titleRadius:         '圆角尺度',
  titleElevation:      '深度与层级',
  titleA11y:           '无障碍',
  titleMotion:         '动效与过渡',
  titleDos:            '应当与禁止',
  titleAnti:           '反模式',
...
const tr = (key: keyof typeof I18N) => I18N[key];
...
export function renderPreviewHTML(doc: DesignMDDocument, tokenMap?: TokenNameMap): string {
  ...
  // Build nav links
  const navItems = [
    { id: 'mission',          label: tr('navMission') },
    { id: 'characteristics',  label: tr('navCharacteristics') },
    { id: 'colors',           label: tr('navColors') },
    { id: 'typography',       label: tr('navTypography') },
    { id: 'buttons',          label: tr('navButtons') },
    { id: 'spacing',          label: tr('navSpacing') },
    { id: 'elevation',        label: tr('navElevation') },
    { id: 'accessibility',    label: tr('navA11y') },
    { id: 'motion',           label: tr('navMotion') },
    { id: 'dos-donts',        label: tr('navDosDonts') },
    { id: 'qa',               label: tr('navQA') },
  ];
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .char-grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;margin-top:8px}
  .char-card{padding:24px;border-radius:14px;border:1px solid var(--ring);background:var(--white);position:relative;overflow:hidden}
  .char-card::before{content:"";position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,var(--accent),transparent);opacity:0.5}
  .char-label{font-size:11px;font-weight:700;color:var(--accent);text-transform:uppercase;letter-spacing:0.08em;margin-bottom:12px}
  .char-body{font-size:14px;line-height:1.65;color:var(--black)}
  .char-body-text{margin-top:8px;font-size:13px;color:var(--slate);line-height:1.55}
  .char-chips{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px}
  .char-chip{display:inline-block;padding:4px 12px;background:var(--surface);border:1px solid var(--ring);border-radius:999px;font-size:12px;font-weight:500;color:var(--black);transition:all 150ms ease}
  ...
  @media(max-width:768px){.hero h1{font-size:32px;letter-spacing:-0.96px}.nav-links{display:none}.section{padding:48px 20px}.do-grid{grid-template-columns:1fr}.char-grid{grid-template-columns:1fr}}
</style>
</head>
<body>
...
<section class="hero">
  <h1>${tr('heroAccent')} <span class="accent">${sourceName}</span></h1>
  <p>${tr('heroSub')}</p>
  ${doc.sourceUrl ? `<div class="hero-meta">${tr('heroSource')}: ${doc.sourceUrl} · ${new Date(doc.generatedAt).toLocaleDateString()}</div>` : ''}
</section>
...
<section class="section" id="mission">
  <div class="section-label">${tr('labelMission')}</div>
  <h2 class="section-title">${tr('titleMission')}</h2>
...
<!-- 02 Style & Characteristics -->
<section class="section" id="characteristics">
  <div class="section-label">${tr('labelChar')}</div>
  <h2 class="section-title">${tr('titleChar')}</h2>
  ${renderCharacteristics(doc.sections.visualTheme)}
</section>
...
<section class="section" id="colors">
  <div class="section-label">${tr('labelColors')}</div>
  <h2 class="section-title">${tr('titleColors')}</h2>
...
<section class="section" id="typography">
  <div class="section-label">${tr('labelTypography')}</div>
  <h2 class="section-title">${tr('titleTypography')}</h2>
...
<section class="section" id="buttons">
  <div class="section-label">${tr('labelButtons')}</div>
  <h2 class="section-title">${tr('titleButtons')}</h2>
...
<section class="section" id="spacing">
  <div class="section-label">${tr('labelSpacing')}</div>
  <h2 class="section-title">${tr('titleSpacing')}</h2>
...
<section class="section" id="radius">
  <div class="section-label">${tr('labelRadius')}</div>
  <h2 class="section-title">${tr('titleRadius')}</h2>
...
<section class="section" id="elevation">
  <div class="section-label">${tr('labelElevation')}</div>
  <h2 class="section-title">${tr('titleElevation')}</h2>
...
<!-- 09 Accessibility -->
  <section class="section" id="accessibility">
    <div class="section-label">${tr('labelA11y')}</div>
    <h2 class="section-title">${tr('titleA11y')}</h2>
  ...
  <section class="section" id="motion">
    <div class="section-label">${tr('labelMotion')}</div>
    <h2 class="section-title">${tr('titleMotion')}</h2>
  ...
  <section class="section" id="dos-donts">
    <div class="section-label">${tr('labelDos')}</div><h2 class="section-title">${tr('titleDos')}</h2>
  ...
  <section class="section" id="anti-patterns">
    <div class="section-label">${tr('labelAnti')}</div>
    <h2 class="section-title">${tr('titleAnti')}</h2>
  ...
  <section class="section" id="qa">
    <div class="section-label">${tr('labelQA')}</div>
    <h2 class="section-title">${tr('titleQA')}</h2>
...
<section class="section" id="responsive">
  <div class="section-label">${tr('labelResponsive')}</div>
  <h2 class="section-title">${tr('titleResponsive')}</h2>
  <div class="content-block">${markdownToHTML(doc.sections.responsiveBehavior)}</div>
</section>

<footer class="footer">${tr('footerGen')} <a href="#">DesignPick</a> ${tr('footerOn')} ${new Date(doc.generatedAt).toLocaleDateString()}</footer>
...
// --- Style & Characteristics section ---
function renderCharacteristics(visualThemeMd: string): string {
  // Parse fields from the markdown. We accept either Chinese or English section keys.
  const fields: { key: string; value: string }[] = [];
  const lines = visualThemeMd.split('\n');
  let i = 0;
  while (i < lines.length) {
    const line = lines[i].trim();
    // Format: **Field:** value (single opening **, no closing — same as markdown bold)
    // Also accept: - **Field:** value (with leading bullet)
  ...
  // Map keys for field lookup (both English and Chinese markdown keys)
  ...
  // Try to find each canonical field
  const get = (candidates: string[]) => {
    for (const f of fields) {
      const norm = f.key.toLowerCase();
      if (candidates.some(c => norm === c || norm.includes(c))) return f.value;
    }
    return '';
  };
  const philosophy = get(['philosophy', '设计理念']);
  const tone = get(['emotional tone', '情绪基调']);
  // Key characteristics: try "key characteristics" first, else look for **Key Characteristics**:
  // followed by comma-separated items on the same or next line.
  let characteristics = get(['key characteristics', '核心特征']);

  // If the value looks like just an intro, scan for a Key Characteristics line
  // that lists bullet items in the markdown
  if (!characteristics) {
    let inKey = false;
    const items: string[] = [];
    for (const line of lines) {
      const t = line.trim();
      if (t.match(/^-\s*\*\*Key Characteristics\*\*[:：]/i) ||
          t.match(/^-\s*\*\*核心特征\*\*[:：]/)) { inKey = true; continue; }
      if (inKey) {
        if (t.startsWith('-') && t.match(/^-\s*\*\*/)) break; // next field
        if (t) items.push(t.replace(/^-\s*/, '').replace(/^•\s*/, ''));
      }
    }
    if (items.length) characteristics = items.join(' • ');
  }

  if (!philosophy && !tone && !characteristics) {
    return `<div class="content-block">${markdownToHTML(visualThemeMd)}</div>`;
  }

  const cards: { label: string; body: string; tag: string }[] = [];
  if (philosophy) {
    cards.push({ label: tr('charPhilosophy'), body: philosophy, tag: 'philosophy' });
  }
  if (tone) {
    cards.push({ label: tr('charTone'), body: tone, tag: 'tone' });
  }
  if (characteristics) {
    // Highlight as chips ONLY when there are clear strong separators (·, |, ;, •).
    // A comma like "Sharp, angular edges" is a phrase, not a list — keep it as one chip.
    const hasStrongSep = /[·|;•]/.test(characteristics);
    let chipsHtml = '';
    if (hasStrongSep) {
      const chips = characteristics.split(/[·|;•]/).map(s => s.trim()).filter(s => s.length > 0 && s.length < 40);
      if (chips.length > 1) {
        chipsHtml = `<div class="char-chips">${chips.map(c => `<span class="char-chip">${c}</span>`).join('')}</div>`;
      }
    }
    cards.push({ label: tr('charKey'), body: characteristics, tag: 'key' });
    cards[cards.length - 1] = { ...cards[cards.length - 1], body: chipsHtml + `<div class="char-body-text">${characteristics}</div>` };
  }

  return `
    <div class="char-grid">
      ${cards.map(c => `
        <div class="char-card char-${c.tag}">
          <div class="char-label">${c.label}</div>
          <div class="char-body">${c.body}</div>
        </div>
      `).join('\n')}
    </div>
  `;
